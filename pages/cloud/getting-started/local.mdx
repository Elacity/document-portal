...
2. **Double-click** the downloaded file to open the disk image (a window will appear showing the app)
3. **Open Terminal** (press `Cmd + Space`, type "Terminal", press Enter)
4. **Copy and paste this command** (it will install the app and open it):
...
<Callout type="info">
**Version note:** The volume name includes the version number. If you downloaded a newer release, replace `1.0.0` with the version you downloaded. Run `ls /Volumes/` after opening the `.dmg` to see the exact name.
</Callout>

5. **Done!** The app opens automatically. You only need to do this once - next time just open ElastOS from your Applications folder.

<Callout type="info">
**Don't drag the app manually** - the Terminal command above handles everything: it copies the app to Applications, removes the security restriction, and opens it for you.
</Callout>
